...
Atomics* query_num = new Atomics();//Query id
...
int main(int argc, char** argv) {
    ...
    int index = query_num->PseudoRandomAtomic() % queries_size;//rand() % queries_size;
    ...
    while (curr_time < exit_time)
    {
        if (curr_time >= next_time)
        {
            ...
            index = query_num->PseudoRandomAtomic() % queries_size;//rand() % queries_size;
