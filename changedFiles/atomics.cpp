...
//Used for having a determenistic sequence of query ids
        int PseudoRandomAtomic()
        {
          int local_seed = 0;
           do {
               local_seed = count_.load(std::memory_order_acquire);

               } while (!count_.compare_exchange_weak(
                  local_seed,
                  (1103515245 * local_seed + 12345) & 0x7fffffff,
                  std::memory_order_acq_rel,
                  std::memory_order_relaxed));

           return (1103515245 * local_seed + 12345) & 0x7fffffff;
         }
