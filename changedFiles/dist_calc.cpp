...
#pragma omp parallel for
    ...
    // FOR LOOP FOR DETECTING CORRUPTION IN RESPONSE
    volatile int dontOptimize=0;
    for(int i=0;i<min_index;i++){dontOptimize++;}
...
//ADD THIS
uint32_t DistCalc::get_knn_value() {
    // using .at() for bounds-checking; will throw std::out_of_range if bad
    return knn_all_queries_[0][0];
}
