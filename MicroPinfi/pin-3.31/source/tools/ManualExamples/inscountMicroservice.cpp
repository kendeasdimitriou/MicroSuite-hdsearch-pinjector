...
VOID ThreadLock (THREADID threadid)//proccess id
{
  PIN_GetLock(&pinLock, threadid + 1);
    ...
    if(tdata && tdata->inject) {
      std::cout << "proc 0 [0] 1.1 1 instructions: ";
    }
}
...
void GetOpCode(VOID *ip, UINT32 size,THREADID tid) {
    ...
    if(tdata && tdata->inject) {
Queries<< tdata->queryId<< std::endl;
        // === Εκτύπωση ρουτίνας ===
//        RTN rtn = RTN_FindByAddress((ADDRINT)ip);
 //       if (RTN_Valid(rtn)) {
 //           std::string rtnName = RTN_Name(rtn);
  //          routines << " [Routine: " << rtnName << "]";
  //      } else {
...
//    routines.open("routines.txt");
    Queries.open("QueriesID_inscount.txt");
