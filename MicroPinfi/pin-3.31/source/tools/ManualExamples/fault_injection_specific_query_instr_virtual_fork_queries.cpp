...
// Addresses of window boundaries
static ADDRINT beginWindowAddr = 0;
static ADDRINT endWindowAddr   = 0;
...
struct ThreadData {////BAZO TO DIKO TOU SEED
    ...
    int queryId;
    int seed;
    CONTEXT savedCtx;   // Saved CPU context for virtual fork
    bool rerun;         // Guard for second pass
};
...
VOID SetInjectTrue(CONTEXT* ctxt,THREADID tid) {
    ThreadData* td = static_cast<ThreadData*>(PIN_GetThreadData(tls_key, tid));
    td->seed = td->queryId;
    if (td->inWindow) return;
    td->inWindow = true;
    td->injection = false;
    td->rerun = false;
    PIN_SaveContext(ctxt, &td->savedCtx);
    //td->seed = td->queryId;
    std::cout << "[VirtualFork] Begin window for query " << td->queryId
              << " (thread " << tid << ")" << std::endl;
}
...
VOID SetInjectFalse(THREADID tid) {
    ThreadData* td = static_cast<ThreadData*>(PIN_GetThreadData(tls_key, tid));
  // td->inWindow=false;
    if (td->inWindow && !td->rerun) {
        // First exit: disable injection and rerun
        td->injection = true;
        td->rerun = true;
        std::cout << "[VirtualFork] End window for query " << td->queryId
                  << " (thread " << tid << ") -- rerunning with injection" << std::endl;
        PIN_ExecuteAt(&td->savedCtx);
    } else if(td->inWindow && td->rerun){
        // Second exit: normal continuation
        td->inWindow = false;
        td->rerun = false; // reset for next window
        std::cout << "[VirtualFork] Completed both passes for query " << td->queryId
                  << " (thread " << tid << ")" << std::endl;
    }
}
...
VOID BeginRequestInterpose(int query_id, THREADID tid)
{
    ...
    tdata->inWindow = false;
    tdata->injection = false;
    tdata->rerun = false;
    tdata->seed = query_id;
...
VOID ThreadLock(THREADID threadid) // process id
{
    PIN_GetLock(&pinLock, threadid + 1);
    ThreadData* tdata = static_cast<ThreadData*>(PIN_GetThreadData(tls_key, threadid));
    if (tdata && tdata->inWindow) {
        // Υποθέτουμε ότι το x έχει ήδη την τιμή 1..4
       int fileIndex = 0;
       if( tdata->injection == true){
         fileIndex = 1;
       }else{
         fileIndex = 0;
       }
...
//injection_commands <<"inside1";
    ThreadData* tdata = static_cast<ThreadData*>(PIN_GetThreadData(tls_key, tid));
    if(tdata && tdata->inWindow && tdata->injection == true) {//Ελεγχος εδω, γιατι το injection window (δηλαδή το flag tdata->inject) μπορεί να αλλάζει δυναμικά κατά την εκτέλεση
...
VOID InstructionInstrumentation(INS ins, VOID *v) {
    ...
    if (addr == beginWindowAddr) {
        INS_InsertCall(ins, IPOINT_BEFORE, AFUNPTR(SetInjectTrue),
                       IARG_CONST_CONTEXT, IARG_THREAD_ID, IARG_END);
    } else if (addr == endWindowAddr) {
        INS_InsertCall(ins, IPOINT_BEFORE, AFUNPTR(SetInjectFalse),
