...
VOID SetInjectTrue(THREADID tid) {
    ...
    td->inject = true;
  //  CONTEXT *saved = new CONTEXT;//
   // PIN_SaveContext(ctxt, saved);//
...
//        PIN_ReleaseLock(&globalLock);
 //       PIN_InitLock(&globalLock);
  ...
  //      PIN_InitLock(&pinLock);//
     //    PIN_ExecuteAt(saved);
        // ακριβής επανεκκίνηση, αν χρειάζεται
        return;  // το child συνεχίζει αμέσως στο window
    }
    // parent
    x=2;
    td->child_pid = pid;//
  ...
  if (td->child_pid > 0) {
        int status = 0;
        waitpid(td->child_pid, &status, 0);
        std::cout << "Joined child PID " << td->child_pid
...
/*
  ...
  */  // ο parent συνεχίζει κι αυτός αμέσως στο window
}
...
VOID SetInjectFalse(THREADID tid) {
   ThreadData* td = static_cast<ThreadData*>(PIN_GetThreadData(tls_key, tid));
   td->inject=false;
    // 2) Αν είμαι child (fault_inject==0), τερματίζω αμέσως

    if (x == 1) {
        // _exit τρέχει atomically χωρίς cleanup βιβλιοθήκης,
        // οπότε βγαίνει κατευθείαν το process
        std::cout << "child terminates"<< std::endl;
        _exit(0);
    }
     //if (td->child_pid > 0) {
     //....
...
PIN_UnlockClient();
        ...
        if (!outfile) {
            ...
            outfile <<"Routine: "<<routineName<<" "<<tdata->queryId << " 0x" << ip_str//<<"Routine: "<<routineName<<" " 
                    << " func module insn:";
            for (UINT32 i = 0; i < fetched; ++i) {
                outfile << " "
...
//injection_commands <<"inside1";
    ThreadData* tdata = static_cast<ThreadData*>(PIN_GetThreadData(tls_key, tid));
    if(tdata && tdata->inject && fault_inject == 1) {//Ελεγχος εδω, γιατι το injection window (δηλαδή το flag tdata->inject) μπορεί να αλλάζει δυναμικά κατά την εκτέλεση
...
// IMPORTANT ///INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)ThreadReleaseLock, IARG_END);
///////////////////////////////////////////////////////////////////////////////////////////////////
    if (!isValidInst(ins))
        return;
...
//////////////////////////////////////////////
// Συνάρτηση για ανάγνωση τιμής ελέγχου από αρχείο
//////////////////////////////////////////////

std::ofstream dbgLog;
// Αυτός θα κληθεί *όποτε* το Pin συναντήσει SIGSEGV
// Ο handler πρέπει να επιστρέφει EXCEPT_HANDLING_RESULT
// 1) Σωστή υπογραφή με PHYSICAL_CONTEXT*
EXCEPT_HANDLING_RESULT SigSegvHandler(THREADID tid,
                                      EXCEPTION_INFO* pExceptInfo,
                                      PHYSICAL_CONTEXT* physCtxt,
                                      VOID* /*v*/)
{
    // 2a) Πάρε τον κωδικό εξαίρεσης
    EXCEPTION_CODE code = PIN_GetExceptionCode(pExceptInfo);
    if (PIN_GetExceptionClass(code) == EXCEPTCLASS_ACCESS_FAULT) {
        dbgLog << "=== CAUGHT ACCESS FAULT (SIGSEGV) ===\n"
               << "Thread ID: " << tid << "\n";

        // 2b) IP και SP από τον PHYSICAL_CONTEXT
        ADDRINT ip = PIN_GetPhysicalContextReg(physCtxt, REG_INST_PTR);
        ADDRINT sp = PIN_GetPhysicalContextReg(physCtxt, REG_STACK_PTR);
        dbgLog << std::hex
               << "Faulting IP: 0x" << ip << "\n"
               << "Stack Pointer: 0x" << sp << "\n";

        // 2c) Διεύθυνση μνήμης που προκάλεσε το fault
        ADDRINT faultAddr;
        if (PIN_GetFaultyAccessAddress(pExceptInfo, &faultAddr)) {
            dbgLog << "Faulting mem addr: 0x" << faultAddr << "\n";
        }
        dbgLog.flush();
    }
    // Επέτρεψε την προώθηση της εξαίρεσης στο OS
...
//////////////////////////
    ...
    dbgLog.open("pintool_sigsegv.log");

//    PIN_InterceptSignal(SIGSEGV, SignalHandler, 0);
    // Καταχώρηση του handler για εσωτερικές εξαιρέσεις/signals
    PIN_AddInternalExceptionHandler(SigSegvHandler, nullptr);
