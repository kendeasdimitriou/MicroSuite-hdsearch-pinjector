...
struct ThreadData {////BAZO TO DIKO TOU SEED
    ...
    int queryId;
    int seed;
    bool isChild;      // true αν είμαστε στο child process
};
...
VOID SetInjectFalse(THREADID tid) {
    ...
    if (tdata->inject == true) {
        PIN_GetLock(&pinLock, tid + 1);
        std::cout << "PIN: This is Parent code. " << "pid = " << PIN_GetPid() << "parent counter: " << tdata->InstCount << " Query: " << tdata->queryId << std::endl;
        PIN_ReleaseLock(&pinLock);
...
VOID InjectBitFlip(ADDRINT ip, UINT32 regIndex, REG reg, CONTEXT* ctxt, THREADID tid) {
    ...
    if (tdata->trace == false ||
        tdata->inject == false || tdata->queryId == 12345)return; 
