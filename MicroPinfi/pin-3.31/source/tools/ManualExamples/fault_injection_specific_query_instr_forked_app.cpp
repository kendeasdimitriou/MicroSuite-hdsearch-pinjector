...
static int seed=45;  // αρχικά υποθέτουμε parent
static bool injectChild = false;  // αρχικά υποθέτουμε parent
static bool injectParent = false;  // αρχικά υποθέτουμε parent
static int queryId=0;  // αρχικά υποθέτουμε parent
static bool enableFaultInjection = true;
static int fileIndex = 2;  // αρχικά υποθέτουμε parent
...
VOID AfterForkInParent(THREADID tid, const CONTEXT* ctxt, VOID* v) {
    // Ενεργοποίηση FI στο parent
    PIN_GetLock(&pinLock, tid + 1);
    enableFaultInjection = false;
    fileIndex = 2;  // parent θα γράφει σε αρχείο #2
    PIN_ReleaseLock(&pinLock);
    LOG("AfterForkInParent: Fault injection ENABLED\n");
}

VOID AfterForkInChild(THREADID tid, const CONTEXT* ctxt, VOID* v) {
    // Απενεργοποίηση FI στο child
    PIN_GetLock(&pinLock, tid + 1);
    enableFaultInjection = false;
    fileIndex = 1;  // child θα γράφει σε αρχείο #1
    PIN_ReleaseLock(&pinLock);
    LOG("AfterForkInChild: Fault injection DISABLED\n");
}
...
VOID SetInjectTrueParent(THREADID tid) {
    PIN_GetLock(&pinLock, tid + 1);
    injectParent = true;
    PIN_ReleaseLock(&pinLock);
}
VOID SetInjectFalseParent(THREADID tid) {
    PIN_GetLock(&pinLock, tid + 1);
    injectParent = false;
    PIN_ReleaseLock(&pinLock);
}  
...
VOID RoutineInstrumentation(RTN rtn, VOID *v) {
   ...
   // std::cout << "Found FaultInjectionEnd" << std::endl; // Debug output
    ...
    if(name == "FaultInjectionBegin_parent") {
    //std::cout << "Found FaultInjectionBegin" << std::endl; // Debug output
        RTN_Open(rtn);
        // Μετά την FaultInjectionBegin θέτουμε το flag σε true
        RTN_InsertCall(rtn, IPOINT_AFTER, (AFUNPTR)SetInjectTrueParent,
                       IARG_THREAD_ID, IARG_END);
        RTN_Close(rtn);
    }
   ...
   // std::cout << "Found FaultInjectionEnd" << std::endl; // Debug output
        RTN_Open(rtn);
        // Πριν την FaultInjectionEnd θέτουμε το flag σε false
        RTN_InsertCall(rtn, IPOINT_BEFORE, (AFUNPTR)SetInjectFalseParent,
                       IARG_THREAD_ID, IARG_END);
        RTN_Close(rtn);
    }
...
PIN_UnlockClient();
        ...
        if (!outfile) {
            ...
            outfile <<"Routine: "<<routineName<<" " <<queryId << " 0x" << ip_str 
                    << " func module insn:";
            for (UINT32 i = 0; i < fetched; ++i) {
                outfile << " "
...
//injection_commands <<"inside1";
    // Ελέγχουμε το global flag
    if (!enableFaultInjection) {
        return;
    }
    //ThreadData* tdata = static_cast<ThreadData*>(PIN_GetThreadData(tls_key, tid));
    if((PIN_GetPid() != parentPID && injectChild==true)||(PIN_GetPid() == parentPID && injectParent==true)) {//Ελεγχος εδω, γιατι το injection window (δηλαδή το flag tdata->inject) μπορεί να αλλάζει δυναμικά κατά την εκτέλεση
...
// injection_commands <<"xmm"<<i<<" REGISTER";
  //      if(p==1||i>=2){return;}
    ...
    PIN_GetLock(&globalLock, tid);
        ...
        UINT32 inject_bit = ( seed % bound_bit);//want to inject fraction part
...
//    srand(time(0));
    ...
    parentPID = PIN_GetPid();     // θυμόμαστε το PID του parent
...
//   UINT32 fault_inject = KnobFaultInject.Value();
    // Register the instrumentation function
