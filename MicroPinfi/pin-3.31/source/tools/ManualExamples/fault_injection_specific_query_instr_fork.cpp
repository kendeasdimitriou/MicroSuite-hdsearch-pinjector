...
std::ofstream injection_commands1;
...
//                      (VOID*)fpContext->fxsave_legacy._xmms[i]._vec64[0]));
   ...
   PIN_GetLock(&globalLock, tid);
if(x==3){
       injection_commands1 << "Injection at instruction: 0x" << std::hex << ip
       ...
       injection_commands2 << "Injection at instruction: 0x" << std::hex << ip
           << ", Register: " << REG_StringShort(reg) << ", Vector: " << j
           << ", Original Value: 0x" << std::hex << xmmValue
           << ", Mask: 0x" << std::hex << mask
           << ", Injected Value: 0x" << std::hex <<injectedValue
...
int readControlFile() {
    int value = 0;
    ifstream controlFile("control.txt");
...
//    srand(time(0));
   ...
   // loadBlacklistedRoutines("blacklisted_routines.txt");
    ...
    injection_commands1.open("injection_results1.txt", std::ios::out | std::ios::trunc);
    if (!injection_commands1.is_open()) {
        std::cerr << "Error opening results file!" << std::endl;
        return 1;
    }
    injection_commands2.open("injection_results2.txt", std::ios::out | std::ios::trunc);
    if (!injection_commands2.is_open()) {
        std::cerr << "Error opening results file!" << std::endl;
        return 1;
    }
...
if (pid1 == 0) {
        x=1;
     fault_inject= 0; // παιδί 1

} else {
    pid_t pid2 = fork();
    if (pid2 == 0) {
        x=2;
        fault_inject = 0; // παιδί 2
    } else {
        pid_t pid3 = fork();
        if (pid3 == 0) {
        x=3;
            fault_inject = 0; // παιδί 3
        } else {
        x=4;
            fault_inject = 1; // αρχικός γονέας
        }
    }
}
////////////
    // Κάθε διεργασία περιμένει μέχρι το αρχείο "control.txt" να περιέχει την τιμή του δικού της process_id.
    cout << "Process " << x << " (PID " << getpid() 
         << ") waiting for control signal (control.txt = " << x << ")..." << endl;
    
    while (readControlFile() != x) {
        this_thread::sleep_for(milliseconds(500));  // καθυστέρηση για αποφυγή υπερβολικού polling
    }
    
    cout << "Process " << x << " received control signal. Proceeding." << endl;
